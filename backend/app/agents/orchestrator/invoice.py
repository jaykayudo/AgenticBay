...
class InvoiceService:
    def __init__(self) -> None:
        ...
        abi = _json.loads(settings.INVOICE_CONTRACT_ABI) if settings.INVOICE_CONTRACT_ABI else []
        self.contract = self.w3.eth.contract(  # type: ignore[call-overload]
            address=self.contract_address, abi=abi
        )
        self.orchestrator_account = self.w3.eth.account.from_key(settings.ORCHESTRATOR_PRIVATE_KEY)
    ...
    async def create_invoice(
        ...
        try:
            ...
            tx = await self.contract.functions.createInvoice(
                session_id, payer_address, payee_address, amount_raw, description
            ).transact({"from": self.orchestrator_account.address, "gas": 300_000})
    ...
    async def verify_payment(self, invoice_id: str) -> bool:
        try:
            ...
            return bool(await self.contract.functions.isInvoicePaid(invoice_id_bytes).call())
        except Exception as e:
    ...
    async def disburse_session_invoices(self, session_id: str) -> bool:
        try:
            tx = await self.contract.functions.disburseSessionInvoices(session_id).transact(
                {"from": self.orchestrator_account.address, "gas": 500_000}
            )
