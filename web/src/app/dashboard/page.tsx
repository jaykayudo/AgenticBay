...
function MetricDelta({ value, tone }: { value: string; tone: Tone }) {
  const Icon =
    tone === "danger" ? ArrowDownRight : tone === "accent" ? ArrowUpRight : ArrowRight;
...
function VolumeChart() {
  ...
  const points = activitySeries.map((item, index) => {
    ...
    const y =
      chartHeight - paddingY - (item.value / maxValue) * (chartHeight - paddingY * 2);
  ...
  return (
    <div className="mt-6">
      ...
      <div className="mt-6 overflow-hidden rounded-[1.5rem] border border-[var(--border)] bg-[var(--surface-2)] p-4 sm:p-5">
        <svg viewBox={`0 0 ${chartWidth} ${chartHeight}`} className="h-[280px] w-full" aria-hidden="true">
          {yTicks.map((tick) => {
            const y =
              chartHeight - paddingY - (tick / maxValue) * (chartHeight - paddingY * 2);
...
export default function DashboardPage() {
  return (
    <div className="space-y-4 xl:space-y-6">
      ...
      <section
        className="grid gap-4 xl:grid-cols-[minmax(0,1.35fr)_minmax(300px,360px)] xl:gap-6"
      >
        <DashboardPanel id="agents">
          <SectionHeader
            ...
            action={
              <Link
                href="/dashboard/agents/northstar-research/analytics"
          ...
          <div className="mt-6 overflow-x-auto rounded-[1.5rem] border border-[var(--border)]">
            <table className="min-w-full text-left text-sm">
              <thead className="bg-[var(--surface-2)] text-xs uppercase tracking-[0.16em] text-[var(--text-muted)]">
                <tr>
                  ...
                  >
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <Link
                            href={`/dashboard/agents/${agent.analyticsId}/analytics`}
                    ...
                    <td className="px-4 py-4 text-[var(--text-muted)] tabular-nums">{agent.jobs}</td>
