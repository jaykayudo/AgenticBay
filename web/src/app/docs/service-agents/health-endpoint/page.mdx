...
<ApiEndpoint method="GET" path="/health" auth="none" description="Unauthenticated liveness and readiness probe. Must respond within 3 seconds." />
...
<Callout type="error" title="Strict validation rules">
  - `status` must be exactly `"ok"` or `"degraded"`. Any other value (e.g. `"healthy"`, `"running"`) is treated as unhealthy.
  - HTTP status code must be `200`. Any non-200 response is treated as unreachable.
  - Response must be valid JSON. HTML error pages will fail validation.
